...
export class NativeCmp {
  ...
  @ViewChild("openCloseExample", { static: false }) private openCloseExample:
    | CompleterCmp
    | undefined;
  @ViewChild("remoteDataExample", { static: false }) private remoteDataExample:
    | CompleterCmp
    | undefined;

  constructor(completerService: CompleterService, http: HttpClient) {
    this.dataService = completerService
      .local(this.countries, "name", "name")
      .imageField("flag");
    this.dataService2 = completerService
      .local(this.quotes, "nm", "nm")
      .descriptionField("qt");
    ...
    this.dataRemote2 = completerService.remote(null, "title", "title");
